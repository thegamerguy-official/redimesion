...
async function main() {
  ...
  const tenantData: Prisma.TenantCreateInput = { name: 'Logística Elche S.L.' };
  const tenant = await prisma.tenant.create({
    data: tenantData
  });

  console.log('📦 Creando Catálogo de Cajas...');
  const boxTypesData: Prisma.BoxTypeCreateManyInput[] = [
  ...
  const skusData: Prisma.SKUCreateManyInput[] = [
